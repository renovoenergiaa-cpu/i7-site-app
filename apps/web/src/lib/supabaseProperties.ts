...
export function isDummyProperty(p: PropertyDTO | any): boolean {
  ...
  // Apenas filtra o exemplo legado falso do Residencial Mangal Gourmet e mocks
  ...
  if (title.includes('mangal gourmet') || bName.includes('mangal gourmet')) return true;
  if (title.includes('residencial mangal') || bName.includes('residencial mangal')) return true;
  return false;
}
